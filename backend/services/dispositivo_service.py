...
def _validar_que_existar_dispositivo(cur, dispositivo_id) -> None | dict:
...
def _obtener_dispositivo_con_acceso(cur, dispositivo_id, usuario_id, rol) -> dict:
    ...
    dispositivo = _validar_que_existar_dispositivo(cur, dispositivo_id)
...
def regenerar_secret_dispositivo(dispositivo_id) -> dict:
    with get_connection() as conn:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            _validar_que_existar_dispositivo(cur, dispositivo_id)
